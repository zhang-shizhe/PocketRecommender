...
class UserCF:
    ...
    def _get_top_n_users(self, target_user_id, top_n):
        ...
        '''
        target_movies = self.frame[self.frame['user_id'] == target_user_id]['item_id']
        other_users_id = [i for i in set(self.frame['user_id']) if i != target_user_id]
        other_movies = [self.frame[self.frame['user_id'] == i]['item_id'] for i in other_users_id]
        ...
        for movie_id in candidates_movies:
            ...
            for user_data in top_n_user_data:
                if movie_id in user_data['item_id'].values:
                    tmp.append(user_data[user_data['item_id'] == movie_id]['rating'].values[0]/5)
                else:
                    tmp.append(0)
